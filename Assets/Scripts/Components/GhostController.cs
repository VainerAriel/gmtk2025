...
public class GhostController : MonoBehaviour
{
    ...
    {
        if (recordedActions == null || recordedActions.Length == 0) return;
        
        float currentTime = Time.time - ghostStartTime;
        
        // Find the current action index based on time
        int nextIndex = currentActionIndex;
        while (nextIndex < recordedActions.Length && recordedActions[nextIndex].timestamp <= currentTime)
        {
            nextIndex++;
        }
        
        // If we've moved to a new action, update the index
        if (nextIndex > currentActionIndex)
        {
            currentActionIndex = nextIndex - 1;
        }
        
        // If we're at the end, freeze the ghost
        if (currentActionIndex >= recordedActions.Length - 1)
        {
            FreezeGhost();
            return;
        }
        ...
        if (currentActionIndex < recordedActions.Length - 1)
        {
            ...
            PlayerController.PlayerAction nextAction = recordedActions[currentActionIndex + 1];
            
            float timeBetweenActions = nextAction.timestamp - currentAction.timestamp;
            float timeSinceCurrentAction = currentTime - currentAction.timestamp;
            
            if (timeBetweenActions > 0)
            {
                float t = timeSinceCurrentAction / timeBetweenActions;
                Vector3 interpolatedPosition = Vector3.Lerp(currentAction.position, nextAction.position, t);
                
                // Apply ground collision check
                Vector3 finalPosition = CheckGrounded(interpolatedPosition);
                transform.position = finalPosition;
                
                // Update sprite direction
                if (nextAction.position.x > currentAction.position.x)
                {
                    transform.localScale = new Vector3(1, transform.localScale.y, transform.localScale.z);
                }
                else if (nextAction.position.x < currentAction.position.x)
                {
                    transform.localScale = new Vector3(-1, transform.localScale.y, transform.localScale.z);
                }
            }
            ...
            {
                // No time between actions, use current position directly
                Vector3 finalPosition = CheckGrounded(currentAction.position);
                transform.position = finalPosition;
            }
        }
    }
    
    private Vector3 CheckGrounded(Vector3 targetPosition)
    {
